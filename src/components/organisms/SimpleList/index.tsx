...
export default function SimpleList() {
  ...
  const AddAtPosition = () => {
    ...
    const position = selected
      ? selected.position
      : Math.ceil(Math.random() * (copyList.length - 1) + 1);
  ...
  return (
    <ContentContainer>
      <div>
        ...
        <ListInput
          ...
          onChange={(e: React.FormEvent<HTMLInputElement>) => {
            setNumberToAdd(parseFloat(e.currentTarget.value));
          }}
        />
        <p>
      ...
      <Description>{t('studies:linked-list-text')}</Description>
    </ContentContainer>
  );
}
